...
const ParticipateSurvey = () => {
  ...
  const sendAnswer = () => {
    const data = {
      surId: surId,
      answerDTOList: JSON.stringify(answerList)
    }
    console.log(data)
    axios.post("/survey-service/v1/answer/reg",data
    ...
    <>
      ...
      {questionList.map((question, index) => (
        <CCard className="m-3" key={question.queId}>
          ...
          <CCardBody>
            <CFormTextarea onChange={(event) => makeAnswer(question.queId, event)}></CFormTextarea>
          </CCardBody>
        </CCard>
      ))}
